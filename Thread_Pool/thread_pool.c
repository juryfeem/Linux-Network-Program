...
void *adjust_thread(void *threadpool)
{
    ...
    while (!pool->shutdown)
    {
        ...
        /*创建新线程算法︰任务数大于最小线程池个数，且存活的线程数少于最大线程个数时如:30=10 88 40<109*/
        ...
        /*销毁多余的空闲线程算法:忙线程x2小于存活的线程数且存活的线程数大于最小线程数时*/
...
/*向线程池中添加一个任务*/
// threadpool_add(thp，process，(void* )&nun[i]);/*向线程池中添加任务process:小写---->大写*/
...
int main(void)
{
    /* threadpool_t *threadpool_create(int nin_thr_nun，int max_thr_nun，int queue_nax_size); */
    struct threadpool_t *thp = threadpool_create(3, 100, 100); /*创建线程池，池里最小3个线程，最大100 ,队列最大100*/
    printf("pool inited");
    // int *num_= (int *)malloc(sizeof(int)*20);
    int num[20], i;
    for (i = 0; i < 20; i++)
    {
        num[i] = i;
